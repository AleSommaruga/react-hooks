...
I got to know the useEffect hook and a way to make it interact with the useState hook. I learnt how to retrieve a value from a local storage (window.localStorage.getItem/setItem). You can use lazy evaluation by putting the parameter of useState into a function, that is called only if necessary. useEffect is triggered at every page update, but this can be avoided by passing as a second parameter a "dependency array" that specifies which variable changes trigger the re-render.

Finally, you can create a custom hook (i.e., a function) in js and call it inside your components, by convenction make it start with 'use'. It can also become generic by using some serialization/deserialization function, as JSON.stringify and JSON.parse and by making use of the useRef hook in order to manage the change of key.
