...
Lifting state is a technique that allows for sharing the state between components, by moving the state management up to the lowest common ancestor component. The opposite of lifting state is called colocating state and consists of moving the state management of a component down as close as possible to that component, in order to lighten the computational weight.
