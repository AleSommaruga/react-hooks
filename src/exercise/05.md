...
useRef hook can be used to access the DOM (for example to serve external libraries), combined with useEffect with an empty dependency list. Access the ref through the current property and remember to return a cleanup function to destroy the node after unmounting.
