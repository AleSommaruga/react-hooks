...
I learnt how a real-world application works, and the application of useState and useEffect hooks in a more complex context. In particular, I learnt the difference between managed and derived state. I also applied a custom hook (useLocalStorageState) and saw how easy it is to apply it.

The game history feature was very hard: you had to add an history (array of squares) and a currentStep and handle both with useState, and also provide the interface with an ordered list that had to be managed. The tip is to first handle the UI, then the interactivity and the logic. You also had to lift state from Board to Game component.
