...
HTTP requests in React are handled with the useEffect hook, by calling inside it a function with the async/await paradigm (and not applying it directly). I used custom components from a 'pokemon.js' script, and also exploited the useState hook.

In the extra credits I learnt how to manage errors. First with a useState hook that stores the error, then by behaving in different ways: by returning something different if an error occurs, by putting all the state logic in a 'state' object (useful in general), by using an ErrorBoundary component.

The latter can be done by writing a class or by importing it from React: in both cases it is important to handle the reset of the component to avoid locking the webpage. It can be done by setting a key prop to (e.g.) pokemonName and make it reset after each change of that variable, by using the 'resetErrorBoundary' library function (with the advantage that only the ErrorBoundary component is reset and not the inner one), or/and by specifying an array as a resetKeys prop of ErrorBoundary, in order to make the component re-render when a variable in the array is modified.
